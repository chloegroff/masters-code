...
plt.title(f'Model {model_num} Binary Accuracy')
plt.ylabel('Accuracy')
plt.xlabel('Epoch number')
plt.legend(['train', 'val'], loc='upper left')
# plt.show()
# plt.savefig(rf'C:\Users\chloe\DE4\Masters\Figures\BinAcc{model_num}.pdf', dpi =300)

print("Max dice coef:", round(max(history['History']['dice_coef']),4))
print("Max val dice coef:", round(max(history['History']['val_dice_coef']),4))
print("\nMin loss:", round(min(history['History']['loss']),4))
print("Min val loss:", round((history['History']['val_loss'][499]),4))